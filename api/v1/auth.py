...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        user_id = None
        expires_in = 9999
        if request.headers.get("Authorization") and request.headers.get("Refreshing"):
        ...
        if expires_in < 50:
            refresh_token = request.headers.get("Refreshing").split("RefreshToken ")[1]
            user_id, token = get_expired_token_user(refresh_token)

            if user_id is None:
                return jsonify({"error": "Can't authenticate user"}), 401
...
def login():
    ...
    if found is None:
        new_user = User(
            id=user_id, name=user_name, email=user_email, profile_pic=user_picture
        )
