...
def calculate_initial_article_score(article, feed):
    ...
    for user in future_readers:
        ...
        logging.info(
            'Calculating initial score for article_id: %s', article.id)
        logging.info('title: %s', article.title)
        scores = calculate_updated_article_scores(
            article_user_score_association)
        article_user_score_association.score_from_time = \
            scores['score_from_time']
        article_user_score_association.score_from_tags = \
            scores['score_from_tags']
...
def calculate_timerot_score(
        article_date, last_scoring_date, scoring_start_date, score):
    ...
    if article_date_delta.days > 0:
        ...
        and last_scoring_date <= one_hour_ago
    ):
        score = calculate_timerot_score_for_firstday(
            article_date, last_scoring_date, one_hour_ago, score)
    elif (
