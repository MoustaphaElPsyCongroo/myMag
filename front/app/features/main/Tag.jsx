...
export const Tag = ({ tag, likeDislikeData }) => {
  ...
  const handleClickTagLikeDislike = (e) => {
    ...
    if ((icon.classList.contains('fa-thumbs-up') && !tagLiked) ||
      ...
      <fetcher.Form method='post' action='/tags'>
        ...
        >
          <span className='fa-layers fa-fw'>
            ...
            />
            {likeCount > 0 && <span className='like-counter fa-layers-counter fa-layers-top-left fa-fade' style={{ backgroundColor: 'green' }}>{likeCount}</span>}
          </span>
        </button>
        <span className='tag-name' style={{ display: 'inline-block' }}>{tag.name}</span>
        <button
          type='submit'
        ...
        >
          <span className='fa-layers fa-fw'>
            ...
            />
            {dislikeCount > 0 && <span className='dislike-counter fa-layers-counter fa-layers-top-left fa-fade' style={{ backgroundColor: 'Tomato' }}>{dislikeCount}</span>}
          </span>
        </button>
      </fetcher.Form>
    </div>
