...
import { faThumbsUp as likedSolid, faThumbsDown as dislikedSolid } from '@fortawesome/free-solid-svg-icons';
...
export const Article = ({ article }) => {
  ...
  const markAsReadOnScroll = () => {
    if (inView && !articleReadRef.current) {
      ...
      const almostAtBottom = 550 + entry.target.offsetHeight + scrollY > documentHeight;
      if (scrollY > articleOffset || almostAtBottom) {
        const articleId = entry.target.dataset.id;
        console.log(articleReadRef.current);
        console.log('article', entry.target.dataset.title);
  ...
  const handleClickLikeDislike = (e) => {
    ...
    if ((icon.classList.contains('fa-thumbs-up') && !articleLiked) ||
  ...
  return (
    <div className='article' ref={ref} data-id={article.id} data-title={article.title}>
      <Link className='article-link' to={article.link} target='_blank' onClick={markAsReadOnClick}>
        <h3 className='article-title'>{article.title}</h3>
        <p>{article.feed_name}</p>
        {article.image && <img className='article-image' src={article.image} alt='article image' />}
      ...
      <ClientOnly fallback={<p>Loading...</p>}>
        {() => (
          article.tags.map((tag) => {
            return (
              <Tag key={tag.id} tag={tag} likeDislikeData={fetcher.data && fetcher.data[tag.id]} />
            );
          }))}
      </ClientOnly>
